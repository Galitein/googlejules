...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderTags = (tags: string[]) => {
        ...
        tags.forEach(tag => {
            ...
            if (tag === filterTag) { button.classList.add('active'); }
    ...
    const showPrompt = (title: string, defaultValue = ''): Promise<string | null> => {
        return new Promise((resolve) => {
            ...
            promptInput.focus();
            const submitListener = (e: Event) => { e.preventDefault(); cleanup(); resolve(promptInput.value); };
            const cancelListener = () => { cleanup(); resolve(null); };
            const cleanup = () => {
    ...
    const renderFolderTree = (parentId: string | null, level: number): HTMLUListElement => {
        ...
        children.forEach(folder => {
            const li = document.createElement('li');
            li.innerHTML = `<div class="folder-item ${selectedFolderId === folder.id ? 'selected' : ''}" data-folder-id="${folder.id}"><span class="folder-name-wrapper"><span>📁</span><span class="folder-name">${folder.name}</span></span><span class="folder-actions"><button class="btn-icon edit-folder-btn" title="Rename Folder">✏️</button><button class="btn-icon delete-folder-btn" title="Delete Folder">🗑️</button><button class="btn-icon add-subfolder-btn" title="Add Subfolder">➕</button></span></div>`;
    ...
    const renderNotesList = () => {
        ...
        notesInFolder.forEach(note => {
            ...
            noteEl.innerHTML = `<div class="note-item-header"><div class="note-item-title">${note.title}</div><button class="btn-icon delete-note-btn" title="Delete Note">🗑️</button></div><div class="note-item-date"><span>Created: ${formatDateTime(note.created_date)}</span><span>Modified: ${formatDateTime(note.modified_date)}</span></div>`;
    ...
    if (modal) modal.addEventListener('click', (e: MouseEvent) => { if (e.target === modal) closeModal(); });
    if (tagInputContainer) { tagInputContainer.addEventListener('click', (e) => { const target = e.target as HTMLElement; if (target.matches('.tag-delete-btn')) { target.parentElement?.remove(); } else { taskTagsInput?.focus(); } }); }
    if (taskTagsInput) { taskTagsInput.addEventListener('keydown', (e) => { if (e.key === ',' || e.key === ' ' || e.key === 'Enter') { e.preventDefault(); const tagText = taskTagsInput.value.trim(); if (tagText) { const badge = createTagBadge(tagText); tagInputContainer?.insertBefore(badge, taskTagsInput); taskTagsInput.value = ''; } } }); }
    taskForm.addEventListener('submit', async (e: Event) => {
        e.preventDefault();
        if (!taskTitleInput || !tagInputContainer || !taskIdInput) return;
        const title = taskTitleInput.value;
        const tags = Array.from(tagInputContainer.querySelectorAll('.tag-badge')).map(badge => badge.textContent?.slice(0, -1).trim() || '').filter(Boolean);
        ...
        try {
            if (id) { await window.api.updateTask(id, { title, tags }); } else { await window.api.createTask({ title, tags }); }
            closeModal();
            fetchAndRenderTasks();
    ...
    taskList.addEventListener('click', async (e: MouseEvent) => {
        ...
        if (target.matches('.task-checkbox')) { const isChecked = (target as HTMLInputElement).checked; await window.api.updateTask(id, { status: isChecked ? 'completed' : 'pending' }); fetchAndRenderTasks(); }
        if (target.matches('.edit-btn')) { const titleEl = taskItem.querySelector('.task-title'); const tagsEl = taskItem.querySelectorAll('.tag'); if (titleEl) { const title = titleEl.textContent || ''; const tags = Array.from(tagsEl).map(t => t.textContent || '').filter(Boolean); openModal('edit', { id, title, tags }); } }
    ...
    const activateTab = (tabName: keyof typeof views) => {
        ...
        if (viewToShow) { viewToShow.style.display = tabName === 'dashboard' ? 'flex' : 'block'; }
        if (tabName === 'tasks') { fetchAndRenderTasks(); }
        else if (tabName === 'meetingNotes') { fetchAndRenderMeetingsData(); showListView(); }
    };
    ...
    const handleAutoSave = debounce(async () => {
        ...
        if (id) { await window.api.updateNote(id, { title, content }); }
        else { if (selectedFolderId) { const newNote = await window.api.createNote({ title, content, folderId: selectedFolderId }); noteIdInput.value = newNote.id; } else { return; } }
        fetchAndRenderMeetingsData();
    }, 500);
    if (getElem('note-editor-container')) {
        quill = new (window as any).Quill('#note-editor-container', { theme: 'snow', placeholder: 'Description...', modules: { toolbar: [[{ 'header': [1, 2, 3, false] }], ['bold', 'italic', 'underline', 'strike'], [{ 'color': [] }, { 'background': [] }], [{ 'list': 'ordered'}, { 'list': 'bullet' }], ['link'], ['clean']] } });
        quill.on('text-change', handleAutoSave);
    }
    if(noteTitleInput) { noteTitleInput.addEventListener('keyup', handleAutoSave); }
    if (newNoteBtn) { newNoteBtn.addEventListener('click', () => { showEditor(null); }); }
    if (noteListView) {
        noteListView.addEventListener('click', async (e) => {
            ...
            if (target.matches('.delete-note-btn')) { e.stopPropagation(); if (confirm('Are you sure you want to delete this note?')) { await window.api.deleteNote(noteId); fetchAndRenderMeetingsData(); } return; }
            const noteToEdit = meetingsData.notes.find(n => n.id === noteId);
            if (noteToEdit) { showEditor(noteToEdit); }
        });
    }
    if (newFolderBtn) { newFolderBtn.addEventListener('click', async () => { const name = await showPrompt('Enter New Folder Name'); if (name) { await window.api.createFolder({ name, parentId: null }); fetchAndRenderMeetingsData(); } }); }
    if (folderTreeContainer) {
        folderTreeContainer.addEventListener('click', async (e) => {
            const target = e.target as HTMLElement;
            const folderItem = target.closest<HTMLElement>('.folder-item');
            const folderId = folderItem?.dataset.folderId;
            if (target.matches('.add-subfolder-btn')) { e.stopPropagation(); const name = await showPrompt('Enter New Sub-folder Name'); if (name && folderId) { await window.api.createFolder({ name, parentId: folderId }); fetchAndRenderMeetingsData(); } return; }
            if (target.matches('.edit-folder-btn')) { e.stopPropagation(); const currentFolder = meetingsData.folders.find(f => f.id === folderId); if (currentFolder) { const newName = await showPrompt('Enter New Folder Name', currentFolder.name); if (newName && newName !== currentFolder.name) { await window.api.updateFolder(currentFolder.id, newName); fetchAndRenderMeetingsData(); } } return; }
            if (target.matches('.delete-folder-btn')) { e.stopPropagation(); if (confirm('Are you sure you want to delete this folder and all its contents?')) { if (folderId) { await window.api.deleteFolder(folderId); if (selectedFolderId === folderId) { selectedFolderId = null; if (notesListHeader) notesListHeader.style.display = 'none'; if (notesListPlaceholder) notesListPlaceholder.style.display = 'block'; } fetchAndRenderMeetingsData(); } } return; }
            if (folderId) {
                ...
                if (notesListTitle && selectedFolder) { notesListTitle.textContent = `Notes in ${selectedFolder.name}`; }
                showListView();
                fetchAndRenderMeetingsData();
            }
        });
